...
export default function CommunityPage() {
  ...
  const fetchForums = useCallback(
    async (pageNum: number) => {
      ...
      if (loading || !hasMore || (pageNum < page && pageNum !== 0)) {
        console.log(
          `Guard: loading=${loading}, hasMore=${hasMore}, pageNum=${pageNum}, current page=${page}. Returning.`,
        );
        return;
      }
      ...
      try {
        ...
        setForums((prevForums) => {
          ...
          if (pageNum === 0) {
            ...
            console.log(
              `Setting forums for page 0, total: ${newForums.length}`,
            );
            return newForums;
          }
          // 그 외 페이지는 추가
          console.log(
            `Appending forums for page ${pageNum}, total new: ${newForums.length}, prev total: ${prevForums.length}`,
          );
  ...
  ); // page 상태가 변경될 때도 forums 함수가 업데이트되도록 함
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        if (entries[0].isIntersecting && hasMore && !loading) {
          console.log(
            `Intersection observed. Triggering fetch for page: ${page}`,
          );
